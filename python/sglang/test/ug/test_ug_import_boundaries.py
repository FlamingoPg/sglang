...
class TestUGImportBoundaries(unittest.TestCase):
    def test_multimodal_gen_does_not_import_srt_ug(self):
        violations = _find_imports(
            PYTHON_ROOT / "sglang" / "multimodal_gen",
            forbidden_prefixes=("sglang.srt.ug",),
        )
    ...
    def test_ordinary_srt_does_not_import_ug_policy(self):
        violations = _find_imports(
            PYTHON_ROOT / "sglang" / "srt",
            forbidden_prefixes=(
                "sglang.srt.ug.coordinator",
                "sglang.srt.ug.sensenova_u1",
            ),
            skip_dirs={PYTHON_ROOT / "sglang" / "srt" / "ug"},
        )
